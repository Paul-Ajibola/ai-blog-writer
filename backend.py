...
def get_database_url():
    ...
    if not database_url:
        raise ValueError(
            "DATABASE_URL is missing. Please add your Render PostgreSQL External Database URL"
        )
...
class EvidenceItem(BaseModel):
    title: str
    url: str
    published_at: Optional[str] = None    # keep of Tavily provides: DO NOT rely on it
...
# ----------------------------------------------
# 5.) Orchestrator (Plan) Node
# ----------------------------------------------
ORCH_SYSTEM = """You are a senior technical writer and developer advocate.
You job is to produce a highly actionable outline for a technical blog post.
...
- Each task must include:
  ...
  - Ensure the overall plan includes at least 2 of these somewhere:
    * minimal code sketch / MWE (set require_code=True for that section)
  ...
  - Mode open_book:
    ...
    - DO NOT include tutorial/how-to sections unless user explicity asked for that.
    - If evidence is empty or insufficient, create a plan that transparently says "insufficient sources"
    and includes only wht can be supported.
...
def orchestrator_node(state: State) -> dict:
    ...
    mode = model.with_structured_output(Plan)
...
# -------------------------------------------
# 8. Reducer (merge + save) [with image generation]
# -------------------------------------------
...
DECIDE_IMAGES_SYSTEM = """You are an experrt technical editor.
...
graph.add_node("reducer", reducer_subgraph)   # notice that we pass the reducer_subgraph, now.
